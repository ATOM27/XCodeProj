...
//  ObjCStrings
//
//  Created by Eugene Mekhedov on 19.04.16.
...
//        for (NSInteger i = [array3 count]-1; i >=0; i--){
//            [sentence appendString:[array3 objectAtIndex:i]];
//            [sentence appendString:@" "];
//        }
//        [finishArray addObject:sentence];
//       
//    }
//    NSLog(@"%@", finishArray);
    
    NSArray* words = [[mainText lowercaseString] componentsSeparatedByString:@" "];
    NSMutableString* finalMainString = [[NSMutableString alloc] init];
    
    for (NSString* word in words){
        NSInteger wordsLength = [word length] - 1;
        if ([word hasSuffix:@"."])
            wordsLength -= 1;
        
        NSString* firstString = [word substringToIndex:wordsLength];
        NSString* secondString = [[word substringFromIndex:wordsLength] uppercaseString];
        NSString* finalString = [firstString stringByAppendingString:secondString];
        [finalMainString appendString:finalString];
  
            [finalMainString appendString:@" "];
        
    }
    NSLog(@"%@", finalMainString);
    return YES;
}
